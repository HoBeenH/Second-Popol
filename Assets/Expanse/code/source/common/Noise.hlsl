#ifndef EXPANSE_NOISE_INCLUDED
#define EXPANSE_NOISE_INCLUDED

#include "Utilities.hlsl"
#include "Random.hlsl"

/**
 * Static class of utility functions useful in generating and manipulating
 * noise.
 * */
class NoiseUtilities {
  static float remap(float value, float original_min, float original_max, float new_min, float new_max) {
    return new_min + saturate((value - original_min) / (original_max - original_min)) * (new_max - new_min);
  }

  static float3 remap(float3 value, float original_min, float original_max, float new_min, float new_max) {
    return new_min + saturate((value - original_min) / (original_max - original_min)) * (new_max - new_min);
  }

  /* Returns a blue noise value. */
  static float getBlueNoiseSequence(int j, int sequenceLength) {
    return frac((j % sequenceLength) * EXPANSE_GOLDEN_RATIO);
  }
};
