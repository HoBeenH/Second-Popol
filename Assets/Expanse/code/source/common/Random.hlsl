...
class Random {
  ...
  static float random_3_1_seeded(float3 uv, float3 seed) {
    return frac(sin(dot(uv,seed))*43758.5453123);
  }
  static float random_3_1(float3 uv) {
    return random_3_1_seeded(uv,
      float3(EXPANSE_DEFAULT_SEED_X_1, EXPANSE_DEFAULT_SEED_X_2, EXPANSE_DEFAULT_SEED_X_3));
  }

  static float2 random_3_2_seeded(float3 uv, float3 seed_x, float3 seed_y) {
    return frac(sin(float2(dot(uv,seed_x),
      dot(uv,seed_y)))*43758.5453);
  }
  static float2 random_3_2(float3 uv) {
    return random_3_2_seeded(uv,
      float3(EXPANSE_DEFAULT_SEED_X_1, EXPANSE_DEFAULT_SEED_X_2, EXPANSE_DEFAULT_SEED_X_3),
      float3(EXPANSE_DEFAULT_SEED_Y_1, EXPANSE_DEFAULT_SEED_Y_2, EXPANSE_DEFAULT_SEED_Y_3));
  }

  static float3 random_3_3_seeded(float3 uv, float3 seed_x, float3 seed_y, float3 seed_z) {
    return frac(sin(float3(dot(uv,seed_x),
      dot(uv,seed_y),
      dot(uv,seed_z)))*43758.5453);
  }
  static float3 random_3_3(float3 uv) {
  ...
  static float random_4_1_seeded(float4 uv, float4 seed) {
    return frac(sin(dot(uv,seed))*43758.5453123);
  }
  static float random_4_1(float4 uv) {
    return random_4_1_seeded(uv,
      float4(EXPANSE_DEFAULT_SEED_X_1, EXPANSE_DEFAULT_SEED_X_2, EXPANSE_DEFAULT_SEED_X_3, EXPANSE_DEFAULT_SEED_X_4));
  }
};

#endif  // EXPANSE_RANDOM_INCLUDED
