...
Shader "KriptoFX/AE/Decal"
{
	...
	[Toggle(USE_NOISE_DISTORTION)] _UseNoiseDistortion("Use Noise Distortion", Int) = 0
		[Toggle(USE_SIMPLE_DISTORTION)] _UseSimpleDistortion("Use Simple Version", Int) = 0
...
#pragma shader_feature USE_SIMPLE_DISTORTION
...
#if USE_SIMPLE_DISTORTION
		half4 distort = tex2D(_NoiseTex, uvNoise) * 2 - 1;
		half4 tex = tex2D(_MainTex, uvMain + distort.xy / 10 * _DistortScale + _DistortSpeed * _Time.xx);
		half4 tex2 = tex2D(_MainTex, uvMain - distort.xy / 7 * _DistortScale - _DistortSpeed * _Time.xx * 1.4 + float2(0.4, 0.6));
		tex *= tex2;
#else
		half2 mask1 = tex2D(_NoiseTex, uvNoise + _DistortSpeed * _Time.x) * 2 - 1;
		half2 mask2 = tex2D(_NoiseTex, uvNoise - _DistortSpeed * _Time.x * 1.4 + float2(0.4, 0.6)) * 2 - 1;
		half2 mask3 = tex2D(_NoiseTex, uvNoise * 3 + _DistortSpeed * _Time.x) * 2 - 1;
		half2 mask4 = tex2D(_NoiseTex, uvNoise * 3 - _DistortSpeed * _Time.x * 1.25 + float2(0.3, 0.7)) * 2 - 1;
		half2 noiseMask = (mask1 + mask2) * _DistortionSpeedScale.z + (mask3 + mask4) * _DistortScale;

		half4 tex = tex2D(_MainTex, uvMain + noiseMask);
#endif
